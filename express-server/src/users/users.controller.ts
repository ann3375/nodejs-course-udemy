...
import { ValidateMiddleware } from '../common/vaildate.middleware';
import { sign } from 'jsonwebtoken';
import { IConfigService } from '../config/config.service.interface';
import { AuthGuard } from '../common/auth.guard';

// контроллер отвечает за роутинг и котроль входных .вых данных
...
export class UserController extends BaseController implements IUserController {
	constructor(
		...
		@inject(TYPES.ConfigService) private configSercice: IConfigService,
	) {
		super(loggerservice);
		this.bindRouters([
			{
	...
	async login(req: Request<{}, {}, UserLoginDto>, res: Response, next: NextFunction): Promise<void> {
		...
		const jwt = await this.signJWT(req.body.email, this.configSercice.get('SECRET'));
		this.ok(res, { jwt });
	}
